...
> Tech stack: Next.js 15, Drizzle + SQLite (Turso), Tailwind v4, shadcn/ui, Groq, Cohere, local file storage.
...
npx shadcn@latest add select tabs textarea toast badge separator
```
...
### Step 2.1 — Drizzle config (SQLite / Turso)

```
drizzle.config.ts
src/lib/db/
  index.ts          ← Turso/libsql connection
...
2. `resources` — id, topic_id, type, title, description, url, file_url, thumbnail_url, tags, transcript, transcript_status, ai_summary, deleted_at
3. `snippets` — id, resource_id (nullable), title, description, language, code, tags
...
For local development with a file-based SQLite (no Turso account needed):
```env
